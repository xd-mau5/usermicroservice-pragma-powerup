...
public class UserMysqlAdapter implements IUserPersistencePort {
    ...
    public void checkAge(User user){
        personRepository.findById(user.getPerson().getId()).ifPresent(person -> {
            String email = String.valueOf(person.getMail());
            if (email == null) {
    ...
    public void saveEmployee(User user) {
        if (user.getRole().getId().equals(EMPLOYEE_ROLE_ID) || user.getRole().getId().equals(USER_ROLE_ID))
        {
            throw new RoleNotAllowedForCreationException();
        }
        isUserWithPersonAndRoleExists(user);
